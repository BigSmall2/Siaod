...
def build_table(s):
    dic_table = dict() #Словарь
    length = len(s)
    for i in range(length - 1):
        dic_table[s[i]] = length - i - 1    #добавляем число смещения по образу
        if not s[length - 1] in dic_table:
            dic_table[s[length - 1]] = length #Добавляем число смещения для символов которых нет в образе(Для них смещение равно длине образа)
    return dic_table
